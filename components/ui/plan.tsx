import ButtoPrimary from "@/components/ui/button-primary";
...
interface PlanProps{
 ...
 const Plan: FC<PlanProps> = ({ name, duration, price, priceMonth, active}) => {
    ...
    <div 
    className={cn(
      "w-full bg-gray-light/30 p-8  rounded-lg flex flex-col items-center gap-y-10", active && 'relative border-2 border-primary' 
      )}
    >
      {active && (
       ...
       <ButtoPrimary type="button" text="Select Plan" className="border-2 border-primary bg-transparent hover:bg-transparent w-full"/>
    </div>
     );
}
